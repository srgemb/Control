...
typedef struct
{
    ...
    double pressure;     // Annual average local pressure [millibars]
                         ...
                         // Среднегодовая местное давление
    ...
    double azm_rotation; // Surface azimuth rotation (measured from south to projection of
                         ...
                         // Вращение поверхности азимут (измеряется с юга на проекции 
                         // нормали к поверхности на горизонтальной плоскости, отрицательный восток)

    double atmos_refract;// Atmospheric refraction at sunrise and sunset (0.5667 deg is typical)
                         // valid range: -5   to   5 degrees, error code: 16
                         // Атмосферная рефракция на рассвете и закате (0,5667 град типично)
    ...
    double zenith;          //topocentric zenith angle [degrees]
                            //зенитный угол - угол между направлением на солце и нормали к земле
    ...
    double incidence;       //surface incidence angle [degrees] Угол падения к поверхности  [градусов]
